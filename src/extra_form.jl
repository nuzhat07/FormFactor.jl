...
function fun(n1::Int64,l1::Int64,m1::Int64,a1::Float64,n2::Int64,l2::Int64,a2::Float64,q::Float64,k::Int64,k1::Int64,l::Int64)
	n = n1-l1-1
	δ = (a2*n2/a1+ n1)/(n1*a2*n2/a1)
	γ = l1+l2+k+3/2
	β = 2/n1
	#α = 2*l1+1
	ν = l+1/2
	N1 = 2^(2+l1+l2-l-1)/(a2^(3/2+l2)*(n1*n2)^2)*√(gamma(n1-l1)*gamma(n2-l2)*gamma(n1+l1+1)*gamma(n2+l2+1))*(1/(a1*n1))^l1*(a1/n2)^l2
	I1 = (-2*a1/(n2*a2))^k*(-β)^k1*q^l*gamma(l+γ+k1+3/2)/(gamma(k+1)*gamma(n2-l2-k)*gamma(2*l2+k+2)*gamma(k1+1)*gamma(n1-l1-k1)*gamma(2*l1+k1+2)*gamma(l+3/2)*δ^(ν+γ+k1+1))*_₂F₁((ν+γ+k1+1)/2,(ν+γ+k1+2)/2,1+ν,-q^2/δ^2)
	A1 = (1im)^l*(2*l+1)*√(π*(2*l1+1)*(2*l2+1))*wigner3j(l1, l2, l, 0, 0, 0)*wigner3j(l1, l2, l, m1, -m1, 0)
	return N1*I1*A1
end
